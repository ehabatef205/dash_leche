...
export default function DashpageSlider() {
  ...
  const getImages = async () => {
    await Slider.getSliderImages().then(res => {
        ...
        >
          <Container className="my-4  " style={{ justifyContent: "center" }}>
            <div style={{ display: "flex", flexDirection: "row" }}>
              <button className="btn text-light" style={{ backgroundColor: "rgb(80, 192, 169)" }} onClick={() => { navigate("/dash/slider/create") }}><i className="bi bi-plus-circle" color="#fff"></i></button>
              <h3 style={{ marginRight: "35%" }}>Slider images</h3>
            </div>
            <div>
                          ...
                          <div key={key} style={{ margin: "15px", display: "flex", border: "0.5px solid #eee" }}>
                            ...
                            <div style={{ marginLeft: "7%", display: "flex" }}>
                              <button className="btn text-light my-3 mx-3  " style={{ backgroundColor: "rgb(80, 192, 169)" }}
                                onClick={() => { navigate(`/dash/slider/edit/${image._id}`); }}><i className="bi bi-pencil-square" color="#fff"></i></button>
                              <button className="btn text-light my-3 mx-3  " style={{ backgroundColor: "red" }} onClick={() => {
                                Slider.deleteSliderImage(image._id).then(e => {
                                  getImages()
                                })
